## -*- Mode: CPerl -*-
## File: DocClassify::Document::DDCTabs.pm
## Author: Bryan Jurish <moocow@cpan.org>
## Description: document classifier: document (ddc tab-dump)
...
package DocClassify::Document::DDCTabs;
...
our $DEFAULT_ATTR_NAMES = q(text pos lemma);

## %ATTR_XLATE
##  + translation table for ddc attribute names
our %ATTR_XLATE =
  (
   (map {($_=>'text')} qw(Token w Utf8Token u)),
   (map {($_=>'norm')} qw(CanonicalToken v)),
   (map {($_=>'lemma')} qw(Lemma l)),
   (map {($_=>'pos')} qw(POS Pos p)),
  );
...
##  #xdoc => $xmlDoc,      ##-- XML::LibXML::Document object (default: none) : UNUSED
...
sub parseSource {
  ...
  while (defined($_=<$fh>)) {
    ...
    elsif (/^%%\$DDC:index\[([0-9]+)\]=(\S+)\s+(\S+)$/) {
      ($i,$along,$ashort) = ($1,$2,$3);
      $a = $ATTR_XLATE{$along} // $ATTR_XLATE{$ashort} // $along;
      $names[$i] = $a;
      next;
    }
    elsif (/^%%/ || /^\s*$/) {
    ...
    ##-- parse csv lines
    @tok = split(/\t/, $_);
    ++$tf->{join("\t", map {"$names[$_]=$tok[$_]"} grep {defined($names[$_])} (0..$#names))};
    ++$N;
  }
  $sig->{N} = $N;
