## -*- Mode: CPerl -*-
## File: DocClassify::Document::CabXml.pm
## Author: Bryan Jurish <moocow@cpan.org>
## Descript: document classifier: document (DTA::CAB XmlNative or XmlTokWrap)

package DocClassify::Document::CabXml;
...
## %$doc, %opts:
##  ##-- NEW in DocClassify::Document::CabXml
...
sub getCats {
  ...
  foreach $c_node (@{$xdoc->findnodes(join('|',@XML_CAT_XPATHS))}) {
    ...
    push(@$cats, {id=>$c_id,deg=>($c_deg//0),name=>($c_name//'unknown')});
  }

  ##-- insert a dummy category if none were defined
  if (!@$cats) {
    push(@$cats, {id=>1, deg=>1, name=>'dummy'});
  }
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="w">
    <xsl:text>text=</xsl:text><xsl:value-of select="@text|@t"/>
    <xsl:text>&#9;</xsl:text>
    <xsl:text>xlit=</xsl:text><xsl:value-of select="xlit/@latin1Text|@xlit|@u"/>
    <xsl:text>&#9;</xsl:text>
    <xsl:text>norm=</xsl:text><xsl:value-of select="moot/@word"/>
    <xsl:text>&#9;</xsl:text>
    <xsl:text>pos=</xsl:text><xsl:value-of select="moot/@tag"/>
    <xsl:text>&#9;</xsl:text>
    <xsl:text>lemma=</xsl:text><xsl:value-of select="moot/@lemma"/>
    <xsl:text>&#10;</xsl:text>
  </xsl:template>
